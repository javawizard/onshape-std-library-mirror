...
import(path : "onshape/std/smjointtype.gen.fs", version : "422.0");
...
}
{
    ...
    for (var edge in evaluatedEdgeQuery)
    {
    ...
    for (var edge in evaluatedEdgeQuery)
    {
        ...
        originalEntities = append(originalEntities, qOwnedByBody(ownerBody));
        var indexedId = id + unstableIdComponent(index);
        var surfaceId = indexedId + SURFACE_SUFFIX;

        var updatedDefinition = updateDefinitionIfNeedsTrimming(context, edge, definition, edgeToFlangeData);
    ...
    //get originals before the boolean
    var allOriginalEntities = evaluateQuery(context, qUnion(originalEntities));
    ...
    for (var entity in evaluateQuery(context, qOwnedByBody(allSurfaces, EntityType.EDGE)))
    {
        ...
        if (size(attributes) == 0 && size(evaluateQuery(context, qEdgeAdjacent(entity, EntityType.FACE))) == 2)
        {
    ...
    var entitiesToAddAssociations = evaluateQuery(context, qSubtraction(qOwnedByBody(allSurfaces), qUnion(allOriginalEntities)));
    ...
    assignSmAssociationAttributes(context, qUnion(entitiesToAddAssociations));
    updateSheetMetalGeometry(context, id, { "entities" : qUnion(entitiesToAddAssociations) });

}, { oppositeDirection : false, limitType : SMFlangeBoundingType.BLIND});

function updateDefinitionIfNeedsTrimming(context is Context, edge is Query, definition is map, edgeToFlangeData is map) returns map
{
    ...
    for (var i = 0; i < size(edgeVertices.vertices); i += 1 )
    {
        ...
        if (cSysAtVertex != undefined)
        {
            ...
            if (projOnY > -TOLERANCE.zeroAngle && projOnX > -TOLERANCE.zeroAngle)
            {
                moveBase[i] = abs(projOnX) > TOLERANCE.zeroLength; //don't move base point if flush
...
function getFlangeData(context is Context, edge is Query, definition is map) {
    ...
    result.thickness = findThickness(context, adjacentFace);
...
function createAndSolveSketch(context is Context, id is Id, edge is Query, definition is map)
{
    ...
    var thickness = flangeData.thickness;
...
{
    var attr = getAttributes(context, {"entities" : qOwnerBody(face), "attributePattern" : asSMAttribute({})});
