...
export function thicken(context is Context, id is Id, thickenDefinition is map)
precondition
{
...
{
    startFeature(context, id, thickenDefinition);

    // ------------- Set the default parameters ---------------
    if(thickenDefinition.oppositeDirection is undefined)
        thickenDefinition.oppositeDirection = false;

    if(thickenDefinition.operationType == undefined)
       thickenDefinition.operationType = NewBodyOperationType.NEW;
