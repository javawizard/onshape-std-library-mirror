...
export import(path : "onshape/std/toleranceTypes.fs", version : "2931.0");
export import(path : "onshape/std/fittolerancetables.gen.fs", version : "2931.0");
...
export function getToleranceInfo(context is Context, definition is map, field is string, isAngle is boolean) returns ToleranceInfo
{
    ...
    {
        toleranceInfo.toleranceType = ToleranceType.NONE;
        toleranceInfo.usePrecisionOverride = false;
        return toleranceInfo as ToleranceInfo;
    }

    if (toleranceInfo.toleranceType == ToleranceType.FIT_WITH_TOLERANCE || toleranceInfo.toleranceType == ToleranceType.FIT_TOLERANCE_ONLY)
    {
        ...
        const standard = toleranceInfo.toleranceFitInfo.standard;
        const holeclass = toleranceInfo.toleranceFitInfo.holeClass;
        const limits = getFitToleranceLimits(nominalSize, standard, holeclass, true);
        toleranceInfo.lower = lookupTableEvaluate(limits.lower);
        toleranceInfo.upper = lookupTableEvaluate(limits.upper);
    }
