...
export import(path : "onshape/std/smcornerbreakstyle.gen.fs", version : "2770.0");
...
export const sheetMetalCornerBreakAttributeBased = defineSheetMetalFeature(function(context is Context, id is Id, definition is map)
    precondition
    {
...
function getWallIds(context is Context, entityArray is array, definitionVertices is array) returns array
{
    ...
    for (var i = 0; i < size(entityArray); i += 1)
    {
        ...
        {
            throw regenError(ErrorStringEnum.SHEET_METAL_ATTRIBUTE_CORNER_BREAK_UNSUPPORTED_SELECTION);
        }
    ...
    if (size(errorEntities) != 0)
    {
        throw regenError(ErrorStringEnum.SHEET_METAL_ATTRIBUTE_CORNER_BREAK_UNSUPPORTED_SELECTION, qUnion(errorEntities));
    }
    return wallIds;
}
