...
export function modifyFace(context is Context, id is Id, definition is map)
precondition
{
    ...
    if(definition.modifyFaceType == ModifyFaceType.DISTANCE_SPEC || definition.modifyFaceType == ModifyFaceType.ANGLE_SPEC || definition.modifyFaceType == ModifyFaceType.CONCENTRIC)
    {
        ...
        if(definition.oppositeDirection != undefined && definition.modifyFaceType != ModifyFaceType.CONCENTRIC)
        {
